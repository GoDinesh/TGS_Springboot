package com.tgsbhadohi.TGS.classes;

public class Constants {
	public static final String GET_RECORD = "Get Record Successfully";
	public static final String CREATE_RECORD = "Create Record Successfully";
	public static final String UPDATE_RECORD = "Update Record Successfully";
	public static final String DUPLICATE_RECORD = "Duplicate Entry";
	public static final String STUDENT_PROMOTED = "Student Promoted Successfully";
	public static final String UPDATE_STATUS ="Error at the time of status updation";
	public static final String FEES_STRUCTURE_OR_SSM_FEES_NOT_AVAILABLE = "Fees Structure or SSM Fees not available";
	 
	
	public static final String SUCCESS = "SUCCESS";
	public static final String ERROR = "ERROR";
	
	public static final String REQUEST_METHOD_NOT_SUPPORTED = "Request method not supported";
	public static final String SERVICE_NOT_AVAILABLE = "Service not available";
	public static final String FAILURE = "FAILURE";
	public static final String FEES_PAID = "Fees paid successfully";
	
	//Masters Message
	public static final String DUPLICATE_CLASS_CODE = "Duplicate class code.";
	public static final String DUPLICATE_CLASS_NAME = "Duplicate class name.";
	public static final String DUPLICATE_DISCOUNT_REASON = "Duplicate discount reasone";
	public static final String AADHAR_NUMBER_ALREADY_AVAILABLE = "Aadhar Number already available";
	public static final String MOBILE_NUMBER_ALREADY_AVAILABLE = "Mobile Number already available";
	
	
}
