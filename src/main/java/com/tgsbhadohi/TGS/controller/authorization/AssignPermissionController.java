...
public class AssignPermissionController {
  ...
  private ResponseEntity<ResponseModel> getAllAssignPermission() {
    ResponseModel res = new ResponseModel(
      ...
      Constants.SUCCESS,
      false,
      assignPermissionService.getAllAssignPermission()
    );
    return new ResponseEntity<>(res, HttpStatus.OK);
  }

  @PostMapping("/insert")
  private ResponseEntity<ResponseModel> saveAssignPermission(
    @Valid @RequestBody AssignPermission assignPermission
  ) {
    ResponseModel res = new ResponseModel(
      Constants.CREATE_RECORD,
      Constants.SUCCESS,
      true,
      assignPermissionService.saveAssignPermission(assignPermission)
    );
    return new ResponseEntity<>(res, HttpStatus.CREATED);
  }

  @PostMapping("/findbyid")
  private ResponseEntity<ResponseModel> getAssignPermissionById(
    @Valid @RequestBody String roleId
  ) {
    ResponseModel res = new ResponseModel(
      Constants.CREATE_RECORD,
      Constants.SUCCESS,
      false,
      assignPermissionService.getAssignPermissionById(roleId)
    );
    return new ResponseEntity<>(res, HttpStatus.CREATED);
  }
}
